...
const string strMessageMagic = "RADICAL Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // RADICAL: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // RADICAL: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    if (pindexBest->nHeight <= 200)
      ...
      {
         nRewardCoinYear = 200 * MAX_MINT_PROOF_OF_STAKE;
      }
      ...
      {
         nRewardCoinYear = 28 * MAX_MINT_PROOF_OF_STAKE;
      }
      
    else if (pindexBest->nHeight <= 200000)
      {
         nRewardCoinYear = 14 * MAX_MINT_PROOF_OF_STAKE;
      }
      
    else if (pindexBest->nHeight <= 500000)
      {
         nRewardCoinYear = 7 * MAX_MINT_PROOF_OF_STAKE;
      }
...
// RADICAL: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes RADICAL fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // RADICAL: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // RADICAL: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // RADICAL: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // RADICAL: track money supply and mint amount info
...
// RADICAL: total coin age spent in transaction, in the unit of coin-days.
...
// RADICAL: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // RADICAL: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // RADICAL: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // RADICAL: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // RADICAL: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // RADICAL: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // RADICAL: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // RADICAL: getblocks may not obtain the ancestor block rejected
    ...
    // RADICAL: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "RADICAL", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "30/06/16";
        CTransaction txNew;
        txNew.nTime = 1466199562;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1466199583;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3630847;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x3401c8d9989dad010039f20780114695215628254365d9f78a875df9b5fd2baa"));
        block.print();
        ...
        // RADICAL: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // RADICAL: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // RADICAL: tell downloading node about the latest block if it's
